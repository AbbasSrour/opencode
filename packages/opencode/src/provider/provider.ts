  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async "nano-gpt"(input) {
      ...
      for (const id of ids) {
        ...
        const contextLimit =
          typeof context === "number" && Number.isFinite(context) && context > 0 ? context : seed.limit.context
        const outputLimit =
          typeof output === "number" && Number.isFinite(output) && output > 0 ? output : seed.limit.output
