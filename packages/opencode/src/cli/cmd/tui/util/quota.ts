...
export async function fetchQuota(
  ...
  try {
    ...
    if (providerID === "anthropic") {
      response = await fetch(`${sdk.url}/provider/usage/claude`)
    } else if (providerID === "antigravity") {
      response = await fetch(`${sdk.url}/provider/usage/antigravity`)
    } else {
      response = await fetch(`${sdk.url}/provider/usage/gemini-cli`)
    }
