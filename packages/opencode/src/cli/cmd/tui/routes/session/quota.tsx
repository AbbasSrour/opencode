...
type Tab = "antigravity" | "gemini-cli" | "claude" | "none"
...
export function QuotaPrompt(props: { onClose: () => void }) {
  ...
  const anthropicProvider = createMemo(() => {
  ...
  const tabs = createMemo((): Tab[] => {
    ...
    if (anthropicProvider()) result.push("claude")

    return result
  })

  // Current status based on the active tab
  const currentStatus = createMemo(() => {
    if (activeTab() === "none") return "not_configured"
  ...
  // Button options based on the current tab status
  ...
  async function fetchAntigravityQuota() {
    ...
    try {
      const groups = await fetchQuota("antigravity", sdk.client)
  ...
  async function fetchGeminiCliQuota() {
    ...
    try {
      const groups = await fetchQuota("gemini-cli", sdk.client)
  ...
  async function fetchClaudeQuota() {
    ...
    try {
      const groups = await fetchQuota("anthropic", sdk.client)
      if (groups.length === 0 && !anthropicProvider()) {
  ...
  function fetchCurrentTab() {
    if (activeTab() === "antigravity") {
      ...
      void fetchClaudeQuota()
    }
  }

  onMount(() => {
    if (antigravityProvider()) void fetchAntigravityQuota()
    if (geminiCliProvider()) void fetchGeminiCliQuota()
    if (anthropicProvider()) void fetchClaudeQuota()
      ...
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <Show when={activeTab() === "claude"}>
          ...
          <Show when={claudeStatus() === "success"}>
            <box paddingLeft={1} gap={0} minHeight={3}>
              <Show when={claudeQuotas().length === 0}>
                <text fg={theme.textMuted}>{"  No quota data available"}</text>
              </Show>
